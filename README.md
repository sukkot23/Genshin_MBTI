<p align='center'> <img src="https://github.com/user-attachments/assets/975e9e27-3351-4d35-82db-ea2b541e3145" width=500> </p>

# <p align='center'> 티바트 MBTI </p>

<p align='center'><b> Genshin Impact : Teyvat MBTI(Myers-Briggs Type Indicator) Test </b></p>

<p align='center'><b> 원신 : 티바트 MBTI 테스트 </b></p>

<br>

<p align='center'> MBTI 테스트를 원신 테마에 맞게 만든 웹페이지 입니다 </p>

<p align='center'> 누구나 자유롭게 테스트 해볼 수 있으며, 자신의 MBTI가 무엇인지</p>

<p align='center'> 어떤 캐릭터와 같은 MBTI 인지 확인해보세요 </p>

<br>

<p align='center'> <img src="https://github.com/user-attachments/assets/dd2cc873-4032-467e-8564-4facf96df502" width=900> </p>

<p align='center'>
  <img src="https://github.com/user-attachments/assets/9e0439bd-289e-4d75-a581-4fcdf868ca86" width='30%'>
  <img src="https://github.com/user-attachments/assets/5addb453-9989-4188-b02c-79681cc583de" width='30%'>
  <img src="https://github.com/user-attachments/assets/7e192cde-6206-49c2-9b2b-4b7d40e5ec52" width='30%'>
</p>





<p align='center'><a href="https://www.mbti.twontree.com/"> <img src="https://github.com/user-attachments/assets/38e1ddbb-93f0-48b0-aacb-52643eb3afb9" height='80'> </a></p>



<br>

```
  ㆍ총 40가지 질문에 응답하여 당신의 MBTI를 확인해보세요

  ㆍ1.0 ~ 5.8까지 원신 캐릭터를 만나 볼 수 있습니다
    * 콜라보 캐릭터, 노드크라이 캐릭터는 현재 추가되지 않았습니다



   * 중립 선택지만 선택할 경우, 검사 결과가 올바르게 나타나지 않을 수 있습니다

   * 다른 MBTI 결과임에도 불구하고, 동일한 일러스트가 나올 수 있습니다
       예시) 라이덴 쇼군 : ISTJ / 라이덴 에이 : INTJ

   * 버그 제보, 개선 사항 등 기타 문의은 해당 프로젝트의 이슈(Issues) 또는 게시된 게시물의 댓글로 문의바랍니다
```

```
  현재 웹페이지 버전 : 1.0.1
  지원 언어 : 한국어 (ko-kr)
 

  제작기간 : 2024.12.21 ~ 2025.12.09
  

  ㆍ 본 웹페이지에 사용된 일부 이미지의 저작권은 HoYoverse 에 있습니다
  ㆍ 본 웹페이지 또는 생성된 이미지는 2차 창작이 불가능 합니다

  ㆍ 본 웹페이지에서 생성된 이미지는 자유롭게 어느곳에 업로드 가능합니다
  ㆍ 업로드 시 출처를 남겨주시면 다음 프로젝트 개발에 많은 도움이 됩니다


    Copyright 2025. SUKKOT All Rights Reserved.
    © All rights reserved by HoYoverse. Other properties belong to their respective owners
```

<br>

## 업데이트 내역

- 1.0.0 (2025-12-09) : 최초 웹페이지 업로드
- 1.0.1 (2025-12-09) : Css 정렬 문제 해결

<br>
<br>

## 홈페이지 (Homepage)
- <a href='https://www.twontree.com'> 투앤트리(Twontree) </a>
- <a href='https://www.mbti.twontree.com/'> 티바트 MBTI (Teyvat MBTI) </a>

<br>
<br>

## 리소스 (Resources)

 - <a href='https://github.com/mholt/PapaParse'> PapaParse </a>
 - <a href='https://goorm-sans.goorm.io/'> 구름 산스 (goorm-sans) </a>
 - <a href='https://x.com/dispe1972?s=20'> 보그보그디자인 (bogbogdesign) </a>

<br>
<br>

## 도움을 주신 분들 (Support for Project)

- 트바 (ChocoB)





